...
static void IdealGasLaw(bool* p_open)
{
    ...
    //Creating the object piston and passing enum to indicate which specie will be inside the piston
    ...
    // Primitives and will be used to position and fix the objects that will be drawn
    ImGui::Text("States");
    static float sz = 220.0f;
    static float thickness = 6.0f;

    //variables that control the heat evolution and the color change
    static float HeatEvolved = 0.0f;
    static float SurroundingPressure = 1;
    static float HeatColor = 0;
    static float ColdColor = 0;

    //Creating the colors that will be used on the renderer
    ...
    ImVec4 Heat_col = ImVec4(0.4157f + HeatEvolved/55.0f, 0.333, 0.3411f, 0.95f);
    ImVec4 Cold_col = ImVec4(0.4157f + ColdColor, 0.333f - ColdColor, 0.3411f - ColdColor, 0.95f);
    static ImVec4 Stopper_col = ImVec4(0.302f, 0.431f, 0.733f, 0.95f);
    static ImVec4 Insulator_col = ImVec4(0.616f, 0.765f, 0.855f, 0.85f);
    static ImVec4 Work_col = ImVec4(0.929f, 0.875f, 0.702f, 0.85f);
    //ImGui::DragFloat("Size", &sz, 0.2f, 2.0f, 452.0f, "%.0f");
    //ImGui::DragFloat("Thickness", &thickness, 0.05f, 1.0f, 15.0f, "%.02f");
    static float raise = 0; //Variable that indicates how much the piston rose



    //Prompting user to choose process and set it to the piston object
    ...
    HeatColor = 0; //This will control the color of the specie when heat is added or removed
    ...
    if (current_state == Piston::process::constant_pressure)
    {
        ...
        piston.AddHeat(HeatEvolved * 1000, Piston::process::constant_pressure); //Calculating the variable changes when heat is added
        raise = piston.returnChangeInPistonHeight(); //Update the state of the piston
        float th = thickness;
        //Draw and update the piston position
        ...
        //Variables that will be used to update the position of the units when window is interacted with
        x = p.x + 4;
        y += sz + spacing;
    ...
    //This will insure that the Unit will be not clipped out when moving the window
        ...
        draw_list->AddRectFilled(ImVec2(x + th - 2.0f, y + sz + 6 - raise + 100), ImVec2(x + sz - th + 3.0f + 100, y + sz + 40 + 153), Air_col32, 0);
        graph.Draw(draw_list, Text_col32);
        //Constructs and draws the graph's function
        graph.BuildLinesVector(piston, Graph::state::constant_volume);
        graph.DrawLines(draw_list, piston);

        //Draws the state information of the piston 
        draw_list->AddText(ImVec2(x + th + 10, y + sz + 20 + 100), Text_col32, piston.returnVolumeText().c_str());
        draw_list->AddText(ImVec2(x + th + 180, y + sz + 20 + 100), Text_col32, piston.returnPressureText().c_str());
        draw_list->AddText(ImVec2(x + th + 10, y + sz + 40 + 100), Text_col32, piston.returnTemperatureText().c_str());
        draw_list->AddText(ImVec2(x + th + 180, y + sz + 40 + 100), Text_col32, piston.returnMolesText().c_str());
	...
	//This will insure that the Unit will be not clipped out when moving the window
        ...
        ImU32 Work_col32 = ImColor(Work_col);
        //Variables that will control the position of the units
        float x = p.x + 100.0f, y = p.y + 200.0f, spacing = 15.0f;
        float x_board = p.x + 5, y_board = p.y + 20, board_spacing = 15.0f;
        //Creation of the graph
        Graph graph(x + 370, y + 300, x + 720, y - 50, 2); //Create Graph

        //Updating the state of the piston
        piston.MolesChange(4.5, Piston::process::constant_temperature);
        ...
        //drawing the state information of the piston
        draw_list->AddText(ImVec2(x + th + 10, y + sz + 20 + 100), Text_col32, piston.returnVolumeText().c_str());
        draw_list->AddText(ImVec2(x + th + 180, y + sz + 20 + 100), Text_col32, piston.returnPressureText().c_str());
        draw_list->AddText(ImVec2(x + th + 10, y + sz + 40 + 100), Text_col32, piston.returnTemperatureText().c_str());
        draw_list->AddText(ImVec2(x + th + 180, y + sz + 40 + 100), Text_col32, piston.returnMolesText().c_str());
