...
void VaporPressureCalculator::ShowVaporCalculator(bool *p_open)
{
	...
	{
		VaporPressure = application.CalculateVaporPressure(temperature, specie_number, Antoine_A, Antoine_B, Antoine_C);
...
float VaporPressureCalculator::CalculateVaporPressure(float temperature, unsigned specie_number, float Antoine_A, float Antoine_B, float Antoine_C)
{
	std::cout << Antoine_A << std::endl << Antoine_B << std::endl << Antoine_C << std::endl;
	return (pow(10, Antoine_A - Antoine_B/( (temperature - 273.15) + Antoine_C)));
}
